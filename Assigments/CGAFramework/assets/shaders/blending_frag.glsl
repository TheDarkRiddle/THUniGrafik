...
void main(){
    ...
    //float blendWeight = texture(blendMap, vertexData.blendMapCord).r;
    // convert textures to linear color space
    float a = clamp(vertexData.rawNormal.y, 0, 1);
    vec3 firstColor = invGammaCorrect(texture(materialDiff, vertexData.blendMapCord).rgb);
    vec3 secondTextureColor = invGammaCorrect((texture(secondTexture, vertexData.blendMapCord)).rgb);
    vec3 diffColor = mix(firstColor, secondTextureColor, 1-a); //blendWeight // firstColor, secondTextureColor
